DATA = {'players': {
    'alice': {
        'level': 41, 'total_score': 2300, 'sessions_played': 13,
        'favorite_mode': 'ranked', 'achievements_count': 5
        },
    'bob': {
        'level': 16, 'total_score': 1800, 'sessions_played': 27,
        'favorite_mode': 'ranked', 'achievements_count': 2
        },
    'charlie': {
        'level': 44, 'total_score': 2150, 'sessions_played': 21,
        'favorite_mode': 'ranked', 'achievements_count': 7
        },
    'diana': {
        'level': 3, 'total_score': 2050, 'sessions_played': 21,
        'favorite_mode': 'casual', 'achievements_count': 4
        },
    'eve': {
        'level': 20, 'total_score': 1434, 'sessions_played': 81,
...
def ft_analytics_dashboard(data: dict) -> None:
    ...
    player_scores = {
        name: data['players'][name]['total_score'] for name in data['players']
        }
    print(f"Player scores: {player_scores}")
    score_catg = {
        "high": len(
            [name for name in data['players']
             if data['players'][name]['level'] > 25]
            ),
        "medium": len(
            [name for name in data['players']
             if 15 < data['players'][name]['level'] < 25]
            ),
        "low": len(
            [name for name in data['players']
             if data['players'][name]['level'] < 15])
        }
    print(f"Level categories: {score_catg}")
    achive_count = {
    ...
    regions = {ele['mode'] for ele in data['sessions']}
    print(f"Active modes: {regions}")
    print("")
    ...
    print(f"Top performer: {performer_player[0]} "
          ...
          f"{data['players'][performer_player[0]]['achievements_count']} "
          f"achievements)"
          )
